// =====================================================================
//   SMART PLANT CARE ROBOT  -  v5.2 (MEGA + 3-ZONE + PI SUPERVISOR)
//   Target board : Arduino Mega 2560
// =====================================================================
//
//  v5.2 changes (self-audit fixes vs v5.1). Each marked [FIX AD..AI]:
//   [AD] Alarm screen cycling was BROKEN in v5.1 — it only ever showed
//        screens 1 + ALARM, skipping moisture (0) and light (2). Fixed
//        with separate normalIdx + showAlarmNext state.
//   [AE] Manual servo override now genuinely holds for 60s. v5.1 said
//        it did, but loop() overwrote the angle within 10ms via the
//        automatic fuzzy call. Now gated by `servoManualUntil`.
//   [AF] printFixed1 no longer prints "nan" (which is invalid JSON).
//        Callers must guard with dhtOK/isnan — they already do.
//   [AG] Removed unused <ctype.h> include.
//   [AH] EE_MAGIC reverted to 0xB5. The new pump-hours bytes are
//        validated by their own 0xFFFF sentinel check, so we don't
//        need to invalidate v5.0/v5.1 fail counters on upgrade.
//   [AI] lcdShowTemp now formats to a fixed-width buffer to avoid
//        residual chars when temperature length changes (e.g. -24.5
//        followed by 5.0 used to leave a stray ' ' at position 15).
//
//  Earlier change log (preserved):
//   v5.1: [S..AC] — DHT fallback, signed-zero temp print, robust JSON
//                   parsing, piBuf overflow handling, immediate state
//                   push to Pi, command rejection reasons, alarm
//                   weave (broken in v5.1, fixed in v5.2), single
//                   printFixed1, pump-hours odometer.
//   v5.0: [J..R] — Mega + 3 zones + Pi link.
// =====================================================================
...
// [FIX AE] Manual servo override window
const unsigned long SERVO_MANUAL_OVERRIDE_MS = 60000UL;  // 1 minute
...
// DHT fallback (when sensor broken, plants still get watered)
...
// [FIX AH] Keep EE_MAGIC at 0xB5 so upgrading v5.0/v5.1 -> v5.2
// preserves fail-counters. The new pump-hours bytes have their own
// sentinel check (see eepromLoad).
const int  EE_ADDR_MAGIC      = 0;
const int  EE_ADDR_FAILS0     = 1;       // bytes 1,2,3
const int  EE_ADDR_PUMP_HRS_H = 4;       // high byte (new in v5.1)
...
// [FIX AD] LCD cycling state — separate "next normal screen" tracker
// so the alarm-weave doesn't clobber it.
uint8_t normalScreenIdx = 0;
bool    showAlarmNext   = false;

// [FIX AE] Manual servo override timer
unsigned long servoManualUntil = 0;
...
PumpState lastReportedState = (PumpState)0xFF;
...
// [FIX AF] No "nan" output; caller must guard against NaN.
// Used in both LCD and JSON contexts. Existing callers (lcdShowTemp,
// reportToPi) already check dhtOK which implies !isnan.
...
// [FIX AI] Buffer-based one-decimal formatter for fixed-width LCD lines.
// Writes at most `bufsize-1` chars + null. Returns nothing; on error
// (bufsize too small) the buffer ends up empty.
void formatFixed1(char *buf, size_t bufsize, float v) {
  if (bufsize == 0) return;
  buf[0] = '\0';
  if (isnan(v)) { snprintf(buf, bufsize, "nan"); return; }
  if (v < 0) {
...
// Robust JSON integer field parser; rejects strings/garbage.
...
void eepromLoad() {
  ...
  if (magic == EE_MAGIC) {
    ...
    // Pump-hours odometer; 0xFFFF = uninitialised (from v5.0/v5.1
    // upgrade or fresh EEPROM region) -> reset to 0.
...
// [FIX AI] Fixed-width format. "Temp: %-5s C   " is always 16 chars:
//   "Temp: " (6) + 5-char left-padded number + " C   " (5) = 16.
// Examples (exactly 16 chars):
//   "Temp: 24.5  C   "
//   "Temp: -24.5 C   "
//   "Temp: 5.0   C   "
//   "Temp: -5.0  C   "
...
// ======================== ZONE SELECTION =============================
// Falls back to neutral DHT values when the sensor is broken.
...
void reportToPi() {
  ...
  // [FIX AE] Report manual override status so Pi can show it
  Serial.print(F(",\"servo_manual\":"));
  Serial.print((millis() < servoManualUntil) ? 1 : 0);
  Serial.println('}');
...
void handlePiCommand(const char* line) {
  ...
  if (strncmp(p, "clear_alarm", 11) == 0) {
    ...
    // [FIX AE] Engage manual override so loop()'s automatic fuzzy
    // call doesn't immediately undo the Pi's choice.
    servoManualUntil = millis() + SERVO_MANUAL_OVERRIDE_MS;
    updateServo(ang);
...
void printDebug(int servoAngle) {
  ...
  if (millis() < servoManualUntil) {
    Serial.print(F(" deg (manual override)"));
  } else {
    Serial.print(F(" deg"));
  }
  Serial.println();
...
void setup() {
  ...
  lcd.setCursor(0, 1); lcd.print(F("v5.2 Mega 3-zone"));
  delay(2000);
  lcd.clear();
  ...
  Serial.println(F("System started (v5.2 - Mega, 3 zones, Pi link)."));
}
...
void loop() {
  ...
  // 3) Servo
  // [FIX AE] Respect manual override window — don't fight the Pi.
  int servoAngle = fuzzyServoAngle(lightPercent);
  if (now < servoManualUntil) {
    // Manual override active. Don't touch the servo here; the Pi
    // command already moved it via updateServo(). Allow the detach
    // logic to still run by passing the last commanded angle.
  ...
  if (pumpState == PUMP_IDLE) {
    if (now - lastScreenSwitch > SCREEN_INTERVAL_MS) {
      // [FIX AD] Correct alarm weave: alternate ALARM with the next
      // *normal* screen, where normalScreenIdx independently cycles
      // 0 -> 1 -> 2 -> 0 -> ...
      // Result with alarm:    MOIST, ALARM, TEMP, ALARM, LIGHT, ALARM, ...
      // Result without alarm: MOIST, TEMP, LIGHT, MOIST, ...
      if (anyAlarm()) {
        if (showAlarmNext) {
          screenIndex = 3;                // alarm screen
        } else {
        ...
        screenIndex     = normalScreenIdx;
        normalScreenIdx = (normalScreenIdx + 1) % 3;
        showAlarmNext   = false;          // reset weave for next time
      }
