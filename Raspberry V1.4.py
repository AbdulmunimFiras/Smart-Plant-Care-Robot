#!/usr/bin/env python3
# =====================================================================
#   PLANT CARE PI CONTROLLER  -  v1.4
#   Raspberry Pi supervisor for the Mega-based plant care robot.
#
#   v1.4 — Second-pass fixes (this revision):
#     • Clean shutdown: keep handle to DB writer thread and join() it,
#       so we no longer race queue.empty() against an in-flight INSERT.
#     • Rate limit on water cmds is now tentative: timestamp is only
#       recorded if the enqueue actually succeeded.
#     • Dashboard light reading filters -1 sentinel (was showing "-1").
#     • PLANT_HTTP_HOST / PLANT_HTTP_PORT tolerate empty/bad values
#       with a clear stderr message instead of an import-time crash.
#     • /snapshot.jpg is no longer fetched via query-string api_key
#       (key leaked to history/logs/referer). Dashboard now uses
#       fetch + X-API-Key + createObjectURL.
#     • Camera reopen has a "pending" guard to prevent concurrent
#       VideoCapture leaks under load.
#     • All <button> elements carry type="button" defensively.
#     • db_prune() does an optional WAL checkpoint to limit .db-wal
#       growth on SD cards over time.
#
#   v1.3 — first-pass audit fixes (carried forward; see git history).
...
# Max seconds graceful_shutdown will wait for the DB writer to drain
...
# Handle to the DB writer thread, populated in main(). graceful_shutdown
# joins it so we never sys.exit() while a write is in flight.
db_writer_thread: threading.Thread | None = None


# ---- Zone rate-limit (tentative-then-commit) -------------------------
# v1.3 ticked the rate limit at the *check* step, which meant a user who
# hit a transient "queue full" was locked out for 10s on a command that
# never reached the Mega. v1.4 splits the check and the record: we only
# record `now` if the command actually got enqueued.
_zone_last_water_mono: dict[int, float] = {}
_zone_rate_lock = threading.Lock()


def _zone_rate_ok(zone: int) -> bool:
    with _zone_rate_lock:
        last = _zone_last_water_mono.get(zone, 0.0)
        return (time.monotonic() - last) >= ZONE_COMMAND_COOLDOWN_S


def _zone_rate_record(zone: int) -> None:
    with _zone_rate_lock:
        _zone_last_water_mono[zone] = time.monotonic()
...
def db_prune() -> None:
    ...
    Without an occasional checkpoint, plant_history.db-wal grows
    unbounded on a long-running Pi — small per write, but real over
    weeks. TRUNCATE merges WAL into the main DB and resets the file.
    """
    try:
        ...
        try:
            ...
            # Not fatal — the checkpoint will retry on next write anyway.
...
def db_writer_loop() -> None:
    ...
    Exits only when shutdown_event is set AND the queue is empty.
    graceful_shutdown() joins this thread, so all enqueued events are
...
# ======================== CAMERA =====================================
class Camera:
    """Thin OpenCV wrapper with non-blocking init and reopen.

    VideoCapture(0) can stall for several seconds when no device is
    present. Both the initial open and post-failure reopen run on a
    background thread, with a `_reopen_pending` flag so back-to-back
    snapshot failures don't spawn racing VideoCapture instances (each
    of which would otherwise hold a USB handle until released).
    """
    ...
    def _spawn_reopen(self) -> None:
        """Idempotent: only one reopen attempt runs at a time."""
        with self.lock:
        ...
        try:
            ...
            with self.lock:
                if self.cap is not None:
                    # Someone else won the race; release the loser.
                    try: new_cap.release()
                    except Exception: pass
                    new_cap = None
                    return
                self.cap = new_cap
                new_cap = None   # ownership transferred
    ...
    def snapshot(self) -> bytes | None:
        ...
        with self.lock:
            if self.cap is None:
                # Cache may still be useful while a reopen is pending.
            ...
            if not ok:
                ...
                self.cap = None
                # Spawn reopen *after* releasing the lock to avoid the
                # inner thread blocking on the same lock.
                ...
                return self.last_jpeg
        # Outside the lock: schedule reopen, return the stale frame.
        self._spawn_reopen()
        return stale
...
def api_command():
    ...
    if cmd == "water":
        ...
        # Only commit the rate-limit timestamp if the command actually
        # made it into the queue. Otherwise a transient queue-full
        # would lock the user out for ZONE_COMMAND_COOLDOWN_S seconds
        # on a command the Mega never saw.
        if status == 200:
            _zone_rate_record(zone)
        return resp, status
...
# ======================== DASHBOARD HTML =============================
# Botanical field journal aesthetic. Changes since v1.3:
#  - Light reading filters -1 sentinel
#  - Buttons all carry type="button"
#  - Camera fetched via fetch+blob with X-API-Key header (no key in URL)
...
function applyStatus(d) {
  ...
  // v1.4: also filter the -1 sentinel for light (was leaking "-1").
...
// ----- Events list -----
// Build with createElement / textContent. Never interpolate
// user/device-supplied strings into innerHTML.
...
// ----- Camera -----
// v1.4: Fetch the snapshot via XHR with the API key in a header, then
// turn it into a blob URL. Previously the key was appended to the
// snapshot URL ('/snapshot.jpg?api_key=...'), which would leak into
// browser history, referer headers, and any server-side access log.
...
def graceful_shutdown(signum, frame) -> None:
    ...
    v1.4: instead of polling db_event_queue.empty(), we join the writer
    thread explicitly. That way we never sys.exit() while a row is
    mid-INSERT, and the writer's `finally: conn.close()` is guaranteed
    to run.
    """
...
def main() -> None:
    ...
    # Daemon=True so an unhandled crash doesn't keep the process up,
    # but graceful_shutdown joins it for a clean exit on signal.
