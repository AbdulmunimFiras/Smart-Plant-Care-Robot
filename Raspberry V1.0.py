...
#       over USB (/dev/ttyACM0 @ 115200), parses incoming JSON status
#       lines, and forwards outbound JSON commands from the UI.
#     - SQLite stores time-series history (moisture, temp, humidity,
#       light) and an event log (waterings, faults, alarm transitions).
#     - OpenCV captures camera frames on demand (works with Pi Camera
#       via libcamera bridge, or any USB webcam at /dev/video0).
#
...
#
#   If your Mega shows up as /dev/ttyUSB0 or a different path, edit
#   SERIAL_PORT below. Use `ls /dev/tty*` to find it.
#
#   Autostart on boot (optional):
#     Create /etc/systemd/system/plant-controller.service with:
#       [Unit]
#       Description=Plant Care Pi Controller
#       After=network.target
#       [Service]
#       User=pi
#       WorkingDirectory=/home/pi/plant
#       ExecStart=/home/pi/plant/.venv/bin/python plant_controller.py
#       Restart=on-failure
#       [Install]
#       WantedBy=multi-user.target
#     Then: sudo systemctl enable --now plant-controller
# =====================================================================
...
class SerialBridge(threading.Thread):
    ...
    Auto-reconnects on disconnect (USB cable pull, Mega reset, etc.)
    """
    ...
    def open(self) -> bool:
        try:
            ...
            log.info(f"Opened {SERIAL_PORT} @ {SERIAL_BAUD}")
            db_insert_event("connect", details=f"Connected to {SERIAL_PORT}")
            return True
        except (serial.SerialException, OSError) as e:
            log.warning(f"Could not open {SERIAL_PORT}: {e}")
            self.ser = None
            return False
...
function applyStatus(d) {
  ...
  // Estimate servo (Mega doesn't broadcast it explicitly; we just show slider)
