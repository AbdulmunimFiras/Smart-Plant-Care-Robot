// =====================================================================
//   SMART PLANT CARE ROBOT  -  v4.2 (BUG-FIX RELEASE)
...
//       Fixes a real fuzzy-logic discontinuity at humidity=0/100.
//   [B] EEPROM persistence was incomplete: failure count was loaded
//       but the system always booted into PUMP_IDLE, granting a known
//       bad pump a free attempt after every reset. Now: if the loaded
//       count >= MAX, boot directly into PUMP_ALARM with a deferred
//       baseline (taken on the first valid moisture read).
//   [C] Servo detach for power-saving was promised but never coded.
//       Added a 500 ms post-move detach + on-demand re-attach.
//   [D] lcdShowVerifying() printed variable-width values without
//       guaranteed total width, leaving an orphan 's' on the line
//       when shrinking from "180s" to "5s". Fixed with snprintf to
//       build an exact 16-char line every time.
//   [E] am2302.read() return code was ignored, so a CRC/timeout
//       failure could leave dhtOK=true with stale values. Now the
//       read status is required to be OK before trusting the data.
//   [F] Build error: "'AnalogStat' does not name a type". The Arduino
//       IDE auto-prototypes every function at the top of the sketch,
//       and the prototype for readAnalogAvg() referenced AnalogStat
//       before its definition. Moved the struct to a TYPES section
//       declared before any function.
// =====================================================================
...
#include <EEPROM.h>
#include <avr/wdt.h>
#include <stdio.h>             // snprintf for [FIX D]
...
// Failure detection
const unsigned long VERIFY_TIMEOUT_MS = 10000UL; // 3 min - real soil response
...
const unsigned long SERVO_DETACH_DELAY_MS  = 500UL;   // [FIX C]
...
bool alarmBaselineNeedsInit = false;   // [FIX B] deferred baseline
...
// ======================== TYPES ======================================
// NOTE: This struct MUST be declared before any function that uses it.
// The Arduino IDE auto-generates function prototypes at the top of the
// sketch; if AnalogStat is declared later in the file, the auto-proto
// for readAnalogAvg() references an unknown type and the build fails
// with: "'AnalogStat' does not name a type".
...
//         Old order made trapezoid(0, 0, 0, 30, 50) return 0, which
//         silently zeroed h_dry whenever air humidity was 0%.
...
//  (The AnalogStat type is defined in the TYPES section above.)
...
// [FIX D] Build a fixed-width 16-char line with snprintf so a shrinking
//         numeric value (e.g. left=180 -> 5) cannot leave an orphan 's'.
//         Format max width: "Rise:100% 180s  " = 16 chars exactly.
//                  min width: "Rise:5  %   5s  " = 16 chars exactly.
void lcdShowVerifying() {
  ...
  lcd.setCursor(0, 0); lcd.print(F("Verifying...    "));   // 16 chars
...
//         later. This kills the holding current and the buzzy idle
//         vibration most cheap servos make. Re-attach happens
//         automatically the next time the angle needs to change.
void updateServo(int angle) {
  ...
  // Detach after the move has had time to settle.
...
void runPumpStateMachine() {
  ...
  switch (pumpState) {
    ...
    case PUMP_ALARM: {
      ...
      // [FIX B] If we entered ALARM at boot from EEPROM, the baseline
      //         hasn't been captured yet. Take it on the first valid
      //         moisture reading; otherwise moistureBeforePump=0 would
      //         instantly auto-clear the alarm.
...
void setup() {
  ...
  // [FIX C] Initialize servo state in sync with the actual position.
  myServo.attach(servoPin);
  servoIsAttached = true;
  myServo.write(0);
  lastServoAngle = 0;
  servoMoveTime  = millis();
  delay(500);                          // let it reach 0 deg
  myServo.detach();
  servoIsAttached = false;
  ...
  lcd.setCursor(0, 1); lcd.print(F("AI Fuzzy v4.2"));
  delay(2000);
  lcd.clear();
  ...
  // [FIX B] Honor the persisted failure count: if we already gave up
  //         pumping before the last reset, do NOT pump again. Boot
  //         straight into ALARM state and wait for a manual rescue.
  if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
    pumpState              = PUMP_ALARM;
    alarmBaselineNeedsInit = true;     // baseline taken on first read
  ...
  Serial.println(F("System started (v4.2)."));
...
void loop() {
  ...
  // [FIX E] Trust the sensor only if read() itself reports OK (==0).
  //         Old code accepted whatever get_*() returned, which can be
  //         the previous valid sample on CRC/timeout failure.
  ...
  // 3) Servo (fuzzy, with hysteresis + auto-detach)
